...
NN.prototype.train = function (trainingData) {
    ...
    console.log('training over iterations:', iterations, 'error', error)
}
...
NN.prototype.setupInputLayer = function (input) {
    ...
    for (var n = 0; n < numNodes; n++) {
        ...
        for (var i = 0; i < numIncoming; i++) {
            this.weights[firstHiddenLayerIndex][n][i] = this.getWeight();
        }
    }
}
...
NN.prototype.setupOutputLayer = function (desiredOutput) {
    ...
    for (var n = 0; n < numOutputNodes; n++) {
        ...
        for (var i = 0; i < numIncoming; i++) {
            this.weights[outputLayerIndex][n][i] = this.getWeight();
        }
    }
}
    ...
    for (var layer = 0; layer < optLayers.length; layer++) {
        ...
        if (layer > 0) {
            ...
            for (var n = 0; n < numNodes; n++) {
                ...
                for (var i = 0; i < numIncoming; i++) {
                    this.weights[layer + 1][n][i] = this.getWeight();
                }
            }
        }
    }
...
/*
    Generate a random weight value for a neuron connection 
    (must be different values to ensure neurons move in different directions)
 */
NN.prototype.getWeight = function () {
